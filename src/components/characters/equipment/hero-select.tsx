...
import { CharactersIdsResponseType } from "@/api/types";
...
export const HeroSelect: FC<HeroSelectProps> = ({
  ...
  } = useFetch<CharactersIdsResponseType>({
    url: `${
      userId
  ...
  useEffect(() => {
    const mainCharacter = charactersIdsData?.mainCharacter;
    if (userId && mainCharacter) setCurrentCharacterId(mainCharacter);
  ...
  return (
    <>
      {charactersIdsData &&
        [
          charactersIdsData.mainCharacter,
          ...charactersIdsData.mercenaries,
        ]?.map((id, index) => {
          const asCurrentChar =
            (!currentCharacterId && index === 0) || currentCharacterId === id;
          return (
            <Button
              key={id}
              variant="outline"
      ...
      {(charactersIdsData?.mercenaries.length || 0) < MAX_CHARACTERS_PER_USER &&
      !userId ? (
